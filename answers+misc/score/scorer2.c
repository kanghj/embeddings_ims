...
#define UNASSIGNABLE_SENSE_TAG "UNASSIGNABLE"
#define TYPO_SENSE_TAG "TYPO"
#define PROPER_SENSE_TAG "PROPERNOUN"
...
{boolean invalid_argument_seen = FALSE;
 ...
 if (argc < 4)
  {fnewline (stderr);   
   fprintf (stderr, "This scorer requires three command-line arguments in the");
   fnewline (stderr);   
   ...
   fnewline (stderr);   
   fnewline (stderr);
   fprintf (stderr, "Optionally, the following flags and switches may be appended:");
   fnewline (stderr);
   fnewline (stderr);
 ...
 if (!(invalid_argument_seen))
  ...
  {global_score = 0.0;
   ...
   sense_table = string_table_for (argv [3]);
